...
int main( int argc, char* argv[] )
{
  ...
  if (  treeName == "" )
    {
      std::cerr << "[WARNING]: treeName not provided, using HggRazor" << std::endl;
      treeName="HggRazor";
    }
  ...
  TString triggerCut = " && ( trigger == 1 ) "; //" && ( HLTDecision[82] || HLTDecision[83] || HLTDecision[93] ) ";
      ...
      for ( auto tmp: myVectBinning )
	{
	  ...
	  if ( currentProcess == "signal" )
	      ...
	      for ( int ipdf = 0; ipdf < 60; ipdf++ )
		{
		  ...
		  pdf[ipdf]->SetBinContent( bin, sqrt( pow( pdf[ipdf]->GetBinContent(bin), 2 )));
		}
	    }     
	}
