...
#extra_tag=Moriond_35p9ifb_March20_Summer16_ReMiniAOD_NewSigmaMoverM_Correction_PDFbugFixed_TChiHH_April8_
extra_tag=Moriond_35p9ifb_April19_Summer16_ReMiniAOD_NewSigmaMoverM_Correction_PDFbugFixed_TChiHH_HZ_BR5050_
...
do
    ...
    echo ./CreateDataCardCommands" --inputCF="$indir1$name".txt --outputFile=MakeDataCards_"$extra_tag$name".sh --dataFile="$datafile" --smhFile="$smhfile" --signalFile="$sfile"/SMS-TChiHHHZ_BR5050_"$name"_1pb_weighted.root --sModel="$name"_DataCards_"$extra_tag" --binningFile="$bfile --currentDir=$PWD
